...
export default function Showtimes() {
    ...
    const theatreIDs = [
      1012, 1039, 1038, 1002, 1045, 1031, 1032, 1033, 1013, 1015,
      1016, 1017, 1041, 1018, 1019, 1021, 1034, 1035, 1047, 1022, 1046
    ];
    ...
    useEffect(() => {
      const fetchShowtimes = async () => {
        try {
          ...
          const response = await fetch(`https://www.finnkino.fi/xml/Schedule/?area=${id}&nrOfDays=7`);
          ...
          for (let i = 0; i < showElements.length; i++) {
            ...
            const picture = show.getElementsByTagName('EventSmallImagePortrait')[0].textContent;

            showData.push({ title, theatre, startTime, picture });
    ...
    useEffect(() => {
      ...
      if (selectedTheatres.length > 0) {
        filtered = filtered.filter((show) => selectedTheatres.includes(show.theatre));
      }
    ...
    return (
    <div>
      <ShowtimeSearch placeholder="Search for showtimes..." onSearch={setSearchQuery} />
      <ShowtimeFilter theatres={theatres} selectedTheatres={selectedTheatres} onFilterChange={setSelectedTheatres} />
